...
def test_failed_stage_stops_capture_and_retains_the_original_failure(monkeypatch, stop_fails):
    calls = []

    def handle(request):
        calls.append(request.url.path)
        return httpx.Response(500 if stop_fails and request.url.path == "/stop_profile" else 200)
