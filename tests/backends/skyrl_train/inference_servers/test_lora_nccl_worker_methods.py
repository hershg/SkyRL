...
def test_worker_caches_route_initializes_once_and_stages_received_factors(monkeypatch):
    ...
    monkeypatch.setattr(
        "skyrl.backends.skyrl_train.weight_sync.lora_transport.vllm_adapter.build_vllm_lora_consumer_plan",
        build_plan,
    )
    monkeypatch.setattr(
        "skyrl.backends.skyrl_train.weight_sync.lora_nccl.rendezvous.open_lora_nccl_receiver_session",
        open_session,
    )
    monkeypatch.setattr(
