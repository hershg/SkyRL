    ...
    async def load_lora_adapter(request: Request):
        ...
        if lora_name is None or (lora_path is None and upload_id is None):
            return JSONResponse(
                ...
                content={
                    "object": "error",
                    "message": "missing lora_name/source",
                    "type": "BadRequest",
                },
            )
...
class TestLoRAControlPlane:
    ...
    @pytest.mark.asyncio
    async def test_remote_upload_lora_adapter_accepts_compact_bin(self, client, mock_servers, tmp_path):
        adapter_path = tmp_path / "adapter"
        adapter_path.mkdir()
        (adapter_path / "adapter_model.bin").write_bytes(b"compact-adapter")
        (adapter_path / "adapter_config.json").write_text('{"r": 32}')

        result = await client.load_lora_adapter("lora-upload", str(adapter_path), "remote_upload")

        assert all("adapter_model.bin" in response["sha256"] for response in result.values())

    @pytest.mark.asyncio
    async def test_remote_upload_lora_adapter_rejects_ambiguous_weights(self, client, tmp_path):
        adapter_path = tmp_path / "adapter"
        adapter_path.mkdir()
        (adapter_path / "adapter_model.bin").write_bytes(b"compact-adapter")
        ...
        with pytest.raises(ValueError, match="Expected exactly one LoRA weight file"):
            await client.load_lora_adapter("lora-upload", str(adapter_path), "remote_upload")
