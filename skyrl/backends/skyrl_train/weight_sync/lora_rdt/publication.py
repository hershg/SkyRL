...
def make_lora_rdt_producer_name(layout: LoRABridgeSourceLayout, source_rank: int) -> str:
    """Return a deterministic actor name for one fixed layout and trainer rank."""
    if source_rank < 0:
        raise ValueError(f"lora_rdt source rank must be non-negative, got {source_rank}")
...
class LoRardtPublicationPlanner:
    ...
    def __init__(self, adapter_name: str, source_rank: int, consumer_count: int, namespace: str | None) -> None:
        if consumer_count <= 0:
    ...
    def plan(
        ...
        local_tensors, local_sources = extract_lora_bridge_sources(records, self._source_rank)
        sources = tuple(
            sorted(
        ...
        if self._layout_digest is None:
            ...
            raise ValueError(f"lora_rdt adapter {self._adapter_name!r} changed its fixed source layout")
        if len(gathered_actor_names) != len({source.source_rank for source in layout.sources}):
            raise ValueError("lora_rdt requires exactly one producer actor name per source rank")
        self._generation += 1
