...
def run_lora_qualification(
    ...
    discriminating_steps = tuple(
        step
        for step, scores in enumerate(state["step_reports"])
        if scores["stale_parity"]["mean_abs"] >= config.mean_atol
    )
            ...
            try:
                with (
                    ...
                    with sdk_checks.measure_phase(phases, f"{phase}/logprobs_before"):
                        sdk_logprobs.score_before_update(trainer, sampler, probes, scores, config.mean_atol)
                        if step == 0:
                            check_initial_adapter(scores, config.mean_atol)
                    ...
                    with sdk_checks.measure_phase(phases, f"{phase}/logprobs_updated"):
                        sdk_logprobs.check_published_update(sampler, probes, scores, config.mean_atol)
...
def _finalize_artifacts(config: LoraQualificationConfig, state: dict) -> None:
    ...
    discriminating_steps = [
        step for step, scores in enumerate(step_reports) if scores["stale_parity"]["mean_abs"] >= config.mean_atol
    ]
    qualification = {
        "passed": state["passed"],
