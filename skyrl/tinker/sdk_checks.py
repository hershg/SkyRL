    ...
    with httpx.Client(base_url=base_url.rstrip("/") + "/", timeout=300) as client:
        ...
        try:
            ...
            try:
                ...
                if failure is None:
                    raise
                failure.add_note(f"Receiver profiler cleanup failed: {cleanup_error}")
    ...
    with httpx.Client(base_url=base_url.rstrip("/") + "/", timeout=600) as client:
        try:
            with measure_phase(report, f"{phase}/trainer_profiler_start") as record:
                ...
                if not status["active"] or status["model_id"] != model_id or status["error"]:
                    raise RuntimeError(f"profiling did not start for {model_id}: {status}")
                claimed = True
                record["status_response"] = status
            yield
        except Exception as error:
            failure = error
            ambiguous_start = isinstance(error, httpx.TimeoutException) or (
                isinstance(error, httpx.HTTPStatusError) and error.response.status_code == 504
            )
            if not claimed and ambiguous_start:
                try:
                    response = client.get("profiling_status")
                    response.raise_for_status()
                    status = response.json()
                    claimed = status["active"] and status["model_id"] == model_id
                except Exception as status_error:
                    error.add_note(f"Could not resolve profiling ownership after timeout: {status_error}")
            raise
        finally:
            if claimed:
                try:
                    with measure_phase(report, f"{phase}/trainer_profiler_stop") as record:
                        ...
                        if status["active"] or status["error"]:
                            raise RuntimeError(f"profiling did not finish cleanly: {status}")
                except Exception as cleanup_error:
                    if failure is None:
                        raise
